...
public class ImageFilterView extends AppCompatImageView {
    ...
    static class ImageMatrix {
        ...
        public void updateMatrix(ImageView imageView) {
            ...
            if (f != 1.0f) {
                saturation(f);
                this.mColorMatrix.set(this.f46m);
                z = true;
            } else {
                z = false;
            }
            ...
            if (f3 != 1.0f) {
                warmth(f3);
                this.mTmpColorMatrix.set(this.f46m);
                this.mColorMatrix.postConcat(this.mTmpColorMatrix);
                z = true;
            }
    ...
    private void init(Context context, AttributeSet attributeSet) {
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0177R.styleable.ImageFilterView);
            int indexCount = obtainStyledAttributes.getIndexCount();
            Drawable drawable = obtainStyledAttributes.getDrawable(C0177R.styleable.ImageFilterView_altSrc);
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0177R.styleable.ImageFilterView_crossfade) {
                    this.mCrossfade = obtainStyledAttributes.getFloat(index, 0.0f);
                } else if (index == C0177R.styleable.ImageFilterView_warmth) {
                    setWarmth(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0177R.styleable.ImageFilterView_saturation) {
                    setSaturation(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0177R.styleable.ImageFilterView_contrast) {
                    setContrast(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0177R.styleable.ImageFilterView_round) {
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRound(obtainStyledAttributes.getDimension(index, 0.0f));
                    }
                } else if (index == C0177R.styleable.ImageFilterView_roundPercent) {
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRoundPercent(obtainStyledAttributes.getFloat(index, 0.0f));
                    }
                } else if (index == C0177R.styleable.ImageFilterView_overlay) {
        ...
        if (f != 0.0f) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                if (this.mViewOutlineProvider == null) {
                    C01731 r7 = new ViewOutlineProvider() {
    ...
    public void setRound(float f) {
        ...
        if (f != 0.0f) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                if (this.mViewOutlineProvider == null) {
                    C01742 r6 = new ViewOutlineProvider() {
                        public void getOutline(View view, Outline outline) {
