...
public final class InternetDomainName {
    ...
    private static final Joiner DOT_JOINER = Joiner.m68on('.');
    private static final Splitter DOT_SPLITTER = Splitter.m81on('.');
    ...
    static {
        ...
        DASH_MATCHER = anyOf;
        PART_CHAR_MATCHER = CharMatcher.javaLetterOrDigit().mo20791or(anyOf);
    }

    InternetDomainName(String str) {
        String lowerCase = Ascii.toLowerCase(DOTS_MATCHER.replaceFrom((CharSequence) str, '.'));
        boolean z = true;
        lowerCase = lowerCase.endsWith(FileUtils.HIDDEN_PREFIX) ? lowerCase.substring(0, lowerCase.length() - 1) : lowerCase;
        ...
        this.parts = copyOf;
        Preconditions.checkArgument(copyOf.size() > MAX_PARTS ? false : z, "Domain has too many parts: '%s'", (Object) lowerCase);
        ...
        this.registrySuffixIndex = findSuffixOfType(Optional.m70of(PublicSuffixType.REGISTRY));
    }
    ...
    public InternetDomainName child(String str) {
        return from(((String) Preconditions.checkNotNull(str)) + FileUtils.HIDDEN_PREFIX + this.name);
    }
