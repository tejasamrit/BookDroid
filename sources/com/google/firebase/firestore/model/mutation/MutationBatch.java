...
public final class MutationBatch {
    ...
    public MaybeDocument applyToRemoteDocument(DocumentKey documentKey, MaybeDocument maybeDocument, MutationBatchResult mutationBatchResult) {
    ...
    public MaybeDocument applyToLocalView(DocumentKey documentKey, MaybeDocument maybeDocument) {
        if (maybeDocument != null) {
            Assert.hardAssert(maybeDocument.getKey().equals(documentKey), "applyToRemoteDocument: key %s doesn't match maybeDoc key %s", documentKey, maybeDocument.getKey());
        }
        for (int i = 0; i < this.baseMutations.size(); i++) {
            ...
            if (mutation.getKey().equals(documentKey)) {
                maybeDocument = mutation.applyToLocalView(maybeDocument, maybeDocument, this.localWriteTime);
            }
        }
        ...
        for (int i2 = 0; i2 < this.mutations.size(); i2++) {
            ...
            if (mutation2.getKey().equals(documentKey)) {
                maybeDocument2 = mutation2.applyToLocalView(maybeDocument2, maybeDocument, this.localWriteTime);
            }
        }
