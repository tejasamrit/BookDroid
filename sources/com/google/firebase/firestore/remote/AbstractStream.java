...
abstract class AbstractStream<ReqT, RespT, CallbackT extends Stream.StreamCallback> implements Stream<CallbackT> {
            ...
            if (Logger.isDebugEnabled()) {
                ...
                for (String next : metadata.keys()) {
                    if (Datastore.WHITE_LISTED_HEADERS.contains(next.toLowerCase(Locale.ENGLISH))) {
                        hashMap.put(next, (String) metadata.get(Metadata.Key.m262of(next, Metadata.ASCII_STRING_MARSHALLER)));
                    }
                }
                if (!hashMap.isEmpty()) {
    ...
    private void close(Stream.State state2, Status status) {
        ...
        Assert.hardAssert(state2 == Stream.State.Error || status.equals(Status.f489OK), "Can't provide an error when not in an error state.", new Object[0]);
        this.workerQueue.verifyIsCurrentThread();
    ...
    public void stop() {
        if (isStarted()) {
            close(Stream.State.Initial, Status.f489OK);
        }
    }
