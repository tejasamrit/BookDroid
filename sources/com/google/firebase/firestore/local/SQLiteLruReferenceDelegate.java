...
class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    public long getSequenceNumberCount() {
        return this.persistence.getTargetCache().getTargetCount() + ((Long) this.persistence.query("SELECT COUNT(*) FROM (SELECT sequence_number FROM target_documents GROUP BY path HAVING COUNT(*) = 1 AND target_id = 0)").firstValue(SQLiteLruReferenceDelegate$$Lambda$1.lambdaFactory$())).longValue();
    }
    ...
    public int removeOrphanedDocuments(long j) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (this.persistence.query("select path from target_documents group by path having COUNT(*) = 1 AND target_id = 0 AND sequence_number <= ? LIMIT ?").binding(Long.valueOf(j), 100).forEach(SQLiteLruReferenceDelegate$$Lambda$3.lambdaFactory$(this, iArr)) != 100) {
                    z = false;
                }
            }
        }
